...
const datasets = {
  ...
  azure: {
    ...
    datasetOptions: ['Azure_EU','Azure_UD'],
    dimensions: {
      'Azure_EU': ['Date', 'Instance Family', 'Instance Size', 'Instance Type', 'Lease Type', 'Operation', 'Reservaton ID', 'Resource ID', 'Transaction Type', 'Usage Type', 'Account ID', 'Account Name', 'Availability Zone', 'Item Description', 'Product Name', 'Region', 'Service Name', 'Resource GUID'],
...
function renderMetricOptions(fileType = 'Combined') {
  ...
  // Remove "Cost(Amortized)" if Azure_EU is selected
  if (cloud === 'azure' && window.currentDataset === 'Azure_EU') {
...
const mappedMetrics = metrics.map(met => {
  if (cloudKey === "Azure") {
    if (dataset === "Azure_EU") {
      return metricMappings.Azure.Azure_EU?.[met] || met;
    }
...
if ((dimensions.length > 0 || tagSelectLines.length > 0) && metrics.length > 0) {
  ...
  orderClause = `    ORDER BY ${metrics.map(m => `${metricAliases[m] || m}`).join(", ")}\n`;
  //orderClause = `    ORDER BY ${metrics.map(m => `\`${metricAliases[m] || m}\``).join(", ")},\n`;
}
...
function updateSelectionPreview() {
  ...
  //enableDrag("dimensionPreview", selectedDimensions);
  //enableDrag("metricPreview", selectedMetrics);  // ✅ Ensure this runs every time
}
